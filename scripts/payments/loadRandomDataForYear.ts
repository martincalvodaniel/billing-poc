 ...
 */
async function loadDataForYear(year: number, baseUrl: string): Promise<void> {
  console.log(`\n${"=".repeat(60)}`);
...
async function main(): Promise<void> {
  ...
  if (args.length < 1) {
    console.error("Usage: npx ts-node scripts/payments/loadRandomDataForYear.ts <year> [baseUrl]");
    console.error(
