...
describe("calculateTotal", () => {
  ...
  it("calculates total from a single concept", () => {
    expect(calculateTotal([{ name: "Item", amount: 100, quantity: 1 }])).toBe(100);
  });

  it("multiplies amount by quantity", () => {
    expect(calculateTotal([{ name: "Item", amount: 50, quantity: 3 }])).toBe(150);
  });
  ...
  it("handles decimal amounts", () => {
    const result = calculateTotal([{ name: "Item", amount: 33.33, quantity: 3 }]);
    expect(result).toBeCloseTo(99.99, 2);
  });

  it("defaults quantity to 1 when not provided", () => {
    // In the function, it uses `c.quantity || 1`
    expect(calculateTotal([{ name: "Item", amount: 100, quantity: 0 }])).toBe(100);
  });
});
