...
- Action button in Monthly Overview opens a read-only modal
...
- **Payment Form Refactoring**: Extracted shared payment form state/handlers into reusable molecules and utilities:
  - **`usePaymentForm` Hook** (`app/month/components/usePaymentForm.ts`): Custom hook managing payment form state (type, date, tag, concepts, VAT, surcharge, client, delivery note), tag autocomplete, concept CRUD, and calculations. Shared between PaymentForm (create mode) and PaymentDetailModal (edit mode). Eliminates 100+ lines of duplicated state/handler logic
  - **`PaymentFormFields` Molecule** (`app/month/components/PaymentFormFields.tsx`): Reusable form fields component rendering all payment inputs (type selector, date, tag with autocomplete, additional fields, concepts, VAT, calculation summaries). Used by both PaymentForm and PaymentDetailModal
  - **`paymentUtils` Utilities** (`app/month/components/paymentUtils.ts`): Shared calculation functions (calculateTotal, calculateVatAmount, calculateSurchargeAmount, calculateNetAmount) and validation helpers (validateConcepts, validateVat, validateSurcharge). Eliminates duplicate calculation logic across both components
  - **Result**: Reduced duplication from ~60% (740 duplicated lines) to modular, testable shared code. Both forms now share consistent validation, calculations, and UI patterns. New forms can reuse these without duplicating
