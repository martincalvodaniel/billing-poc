...
Re-exports pure functions from `src/lib/domain/services/payment-calculator.ts` (99 unit tests via `bun test`):
...
- GETs: `usePayments({ year, month })` from `src/features/<feature>/hooks/usePayments`, `useTags(type)` from `src/features/<feature>/hooks/useTags`.
- Mutations: `useCreatePayment` / `useUpdatePayment` / `useDeletePayment` from `src/features/<feature>/hooks/usePaymentMutations`; `useGenerateInvoice` / `useUploadInvoice` from `src/features/<feature>/hooks/useInvoiceMutations`. Each mutation invalidates the payments cache automatically.
