...
- **Payment Browser** - View filtered transactions in a table (day, type, tag, total, VAT, net amount) with summary cards (total income, outcome, balance) and real-time updates
- **Month Navigation** - Dedicated filter section at the top displays the selected month and payment count. Calendar picker (closes when clicking outside) with prev/next navigation, month grid, year selection, and manual year input. Quick icon button jumps to the current month and disables when already there. Add payment button positioned adjacent to the payment counter for easy access. Automatically navigates to the saved payment's month after creating a new payment. Form date field syncs with calendar selection to match the viewed month
...
- **Year Summary View** - Dedicated yearly page with prev/next/current year controls, inline year picker (grid + manual entry), payment count display, yearly totals, tag donuts, and monthly breakdown cards; top navigation links between monthly list and yearly summary
